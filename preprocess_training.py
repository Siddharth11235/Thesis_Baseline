#Cloned from https://github.com/pritishyuvraj/Voice-Conversion-GAN/blob/master/preprocess_training.py
...
def preprocess_for_training(train_A_dir, train_B_dir, cache_folder):
    ...
    wavs_A = dataset.load_wavs(wav_dir=train_A_dir, sr=sampling_rate)
    wavs_B = dataset.load_wavs(wav_dir=train_B_dir, sr=sampling_rate)
    ...
    f0s_A, timeaxes_A, sps_A, aps_A, coded_sps_A=dataset.world_encode_data(wave=wavs_A, fs=sampling_rate, frame_period=frame_period, coded_dim=num_mcep)
    f0s_B, timeaxes_B, sps_B, aps_B, coded_sps_B=dataset.world_encode_data(wave=wavs_B, fs=sampling_rate, frame_period=frame_period, coded_dim=num_mcep )


    log_f0s_mean_A, log_f0s_std_A = dataset.logf0_statistics(f0s=f0s_A)
    log_f0s_mean_B, log_f0s_std_B = dataset.logf0_statistics(f0s=f0s_B)
    ...
    coded_sps_A_transposed = dataset.transpose_in_list(lst=coded_sps_A)
    coded_sps_B_transposed = dataset.transpose_in_list(lst=coded_sps_B)
...
if __name__ == '__main__':
    ...
    train_A_dir_default = './Baseline_Data/Joni_Mitchell/'
    train_B_dir_default = './Baseline_Data/Don_Mclean/'
    ...
    preprocess_for_training(train_A_dir_default, train_B_dir_default, cache_folder)
